...
def get_efficient_unet_b0(input_shape, training, pretrained=False, block_type='transpose', concat_input=True):
    encoder = get_efficientnet_b0_encoder(input_shape, training=training, pretrained=pretrained)
    model = _get_efficient_unet(encoder, block_type=block_type, concat_input=concat_input)
    return model


def get_efficient_unet_b1(input_shape, training, pretrained=False, block_type='transpose', concat_input=True):
    encoder = get_efficientnet_b1_encoder(input_shape, training=training, pretrained=pretrained)
    model = _get_efficient_unet(encoder, block_type=block_type, concat_input=concat_input)
    return model


def get_efficient_unet_b2(input_shape, training, pretrained=False, block_type='transpose', concat_input=True):
    encoder = get_efficientnet_b2_encoder(input_shape, training=training, pretrained=pretrained)
    model = _get_efficient_unet(encoder, block_type=block_type, concat_input=concat_input)
    return model


def get_efficient_unet_b3(input_shape, training, pretrained=False, block_type='transpose', concat_input=True):
    encoder = get_efficientnet_b3_encoder(input_shape, training=training, pretrained=pretrained)
    model = _get_efficient_unet(encoder, block_type=block_type, concat_input=concat_input)
    return model


def get_efficient_unet_b4(input_shape, training, pretrained=False, block_type='transpose', concat_input=True):
    encoder = get_efficientnet_b4_encoder(input_shape, training=training, pretrained=pretrained)
    model = _get_efficient_unet(encoder, block_type=block_type, concat_input=concat_input)
    return model


def get_efficient_unet_b5(input_shape, training, pretrained=False, block_type='transpose', concat_input=True):
    encoder = get_efficientnet_b5_encoder(input_shape, training=training, pretrained=pretrained)
    model = _get_efficient_unet(encoder, block_type=block_type, concat_input=concat_input)
    return model


def get_efficient_unet_b6(input_shape, training, pretrained=False, block_type='transpose', concat_input=True):
    encoder = get_efficientnet_b6_encoder(input_shape, training=training, pretrained=pretrained)
    model = _get_efficient_unet(encoder, block_type=block_type, concat_input=concat_input)
    return model


def get_efficient_unet_b7(input_shape, training, pretrained=False, block_type='transpose', concat_input=True):
